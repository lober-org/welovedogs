...
import { Star, TrendingUp, DollarSign, Facebook, Instagram, Linkedin } from "lucide-react";
...
export default function VeterinaryProfilePageClient({ veterinarian }: { veterinarian: any }) {
  ...
  useEffect(() => {
    if (vet && vet.currentCauses.length > 1) {
      const interval = setInterval(() => {
        setCurrentImageIndex((prev) => (prev + 1) % vet.currentCauses.length);
      }, 4000);
  ...
  const paginatedTransactions =
    vet?.transactions.slice(
      currentPage * transactionsPerPage,
      (currentPage + 1) * transactionsPerPage
    ) || [];
  const totalPages = Math.ceil((vet?.transactions.length || 0) / transactionsPerPage);

  const storyQA = parseStory(vet.story, "veterinarian");

  if (!vet) {
    return <div className="container py-20 text-center text-black">Veterinarian not found</div>;
  }
        ...
        <div className="mb-6">
          <div className="mb-2 flex flex-wrap items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-base font-semibold text-gray-900">{vet.rating.toFixed(1)}</span>
            </div>
            <div className="flex gap-2">
              {vet.socialMedia.linkedin && (
          ...
          <p className="text-lg font-semibold text-gray-900 mb-1">{vet.clinicName}</p>
          <p className="text-sm text-gray-600 md:text-base">{vet.city}</p>
        </div>
        ...
        <div className="mb-8 mx-auto max-w-4xl">
          <div className="grid gap-4 md:grid-cols-2 md:gap-6">
            <div className="relative aspect-square w-full overflow-hidden rounded-lg bg-gray-100">
              <img
                src={vet.profilePhoto || "/placeholder.svg"}
                alt={vet.fullName}
          ...
          <Tabs defaultValue="dogs" className="w-full">
              ...
              <div className="space-y-6">
                {storyQA.length > 0 ? (
                  storyQA.map((qa, index) => (
                    <div
                      key={index}
                      className="bg-gradient-to-br from-purple-50 to-white rounded-lg border-2 border-purple-200 p-6 shadow-sm"
                    >
                ...
                <div className="mb-6 grid gap-4 md:grid-cols-2">
                  <div className="rounded-lg bg-purple-100 border border-purple-300 p-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-sm text-gray-700">Total Received</p>
                        <p className="text-2xl font-bold text-green-600">
                          ${vet.totalReceived.toLocaleString()}
                        </p>
                      </div>
                    </div>
                  </div>
            ...
            <TabsContent value="updates">
              <div className="space-y-4">
                {vet.latestUpdates.map(
                  (
                    update: { dogName: string; dogImage: string; update: string; date: string },
                    idx: number
                  ) => (
                    <div
                      key={idx}
        ...
        <div className="md:hidden mb-6">
          <Select value={mobileTab} onValueChange={setMobileTab}>
            ...
            <SelectContent>
              ...
              <SelectItem value="story">Their Story</SelectItem>
          ...
          <div className="mt-4">
              ...
              <div className="space-y-4">
                {storyQA.length > 0 ? (
                  storyQA.map((qa, index) => (
                    <div
                      key={index}
                      className="bg-gradient-to-br from-purple-50 to-white rounded-lg border-2 border-purple-200 p-4 shadow-sm"
                    >
            ...
            {/* Financial Overview tab for mobile */}
            {mobileTab === "financial" && (
