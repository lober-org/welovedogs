...
│  ├── Pages & Routes (app/)                                  │
│  │   ├── Homepage (campaign discovery)                       │
│  │   ├── Campaign pages (/donate/[dogId])                    │
│  │   ├── Care provider dashboard                             │
│  │   ├── User profiles (donor, rescuer, shelter, vet)        │
...
│  ├── WalletsKitProvider (Wallet State)                      │
│  ├── SupabaseProvider (Auth & Data)                         │
│  └── SorobanProvider (RPC Client)                           │
│                                                              │
│  Custom Hooks                                                │
│  ├── useWalletsKit, useDonation, usePodPoap                 │
│  ├── useSupabaseAuth, useSupabaseProfile                    │
│  └── useSoroban, useStellarAccount                          │
└─────────────────────────────────────────────────────────────┘
                            │
        ┌───────────────────┼───────────────────┐
        │                   │                   │
...
│   Backend    │  │   Network    │  │   Contracts  │
├──────────────┤  ├──────────────┤  ├──────────────┤
│ PostgreSQL   │  │ Horizon API  │  │ Donation     │
│ Auth         │  │ Wallets Kit  │  │ POD POAP     │
│ Storage      │  │              │  │              │
│ Realtime     │  │              │  │              │
└──────────────┘  └──────────────┘  └──────────────┘
```
...
│   │   ├── page.tsx              # Homepage with campaign discovery
│   │   ├── about/                # About page
│   │   ├── how-we-work/          # How it works page
│   │   ├── care-providers/       # Care providers listing
│   │   ├── donate/               # Donation flow
│   │   │   └── [dogId]/          # Individual dog campaign page
│   │   ├── donation-success/     # Donation confirmation page
│   │   ├── profile/              # User profiles
│   │   │   ├── care-provider/   # Care provider dashboard
│   │   │   │   ├── page.tsx      # Dashboard overview
│   │   │   │   ├── create-dog/   # Create dog profile
│   │   │   │   ├── create-campaign/ # Create campaign
│   │   │   │   └── [dogId]/      # Dog management
│   │   │   ├── donor/            # Donor profile
│   │   │   ├── rescuer/          # Rescuer profile
│   │   │   ├── shelter/          # Shelter profile
│   │   │   └── veterinary/       # Veterinary profile
│   │   ├── register/             # Registration flows
│   │   │   ├── care-provider/   # Care provider registration
│   │   │   └── donor/            # Donor registration
│   │   ├── select-user-type/     # User type selection
│   │   ├── sign-in/              # Authentication
│   │   ├── sign-up/              # Registration
│   │   └── sign-up-success/      # Registration confirmation
│   ├── api/                      # API Routes
│   │   ├── donation/             # Donation endpoints
│   │   │   ├── route.ts          # Process donations
│   │   │   └── stats/            # Donation statistics
│   │   └── pod-poap/             # NFT endpoints
│   │       ├── mint/              # Mint POD NFTs
...
│   ├── DonationButton.tsx        # Donation interface
│   ├── DonationModal.tsx         # Donation modal
│   ├── DonationStats.tsx         # Statistics display
│   ├── donation-story.tsx        # Campaign story component
│   ├── sticky-donation-widget.tsx # Sticky donation widget
│   ├── dog-card.tsx              # Dog card component
│   ├── dog-cards.tsx              # Dog cards grid
│   ├── hero.tsx                  # Homepage hero
│   ├── care-provider-cards.tsx   # Care provider cards
│   └── NFT/                      # NFT-related components
│       ├── PODGallery.tsx        # Proof of Donation gallery
│       └── types.ts              # NFT types
│
├── contexts/                      # React Context Providers
│   ├── WalletsKitContext.tsx    # Wallet state management
│   ├── SupabaseContext.tsx      # Supabase client & auth
│   └── SorobanContext.tsx        # Soroban RPC client
│
├── hooks/                         # Custom React Hooks
│   ├── useWalletsKit.ts          # Wallet operations
│   ├── useDonation.ts            # Donation logic
│   ├── usePodPoap.ts             # NFT operations
│   ├── useSupabaseAuth.ts        # Auth helpers
│   ├── useSupabaseProfile.ts     # Profile management
│   ├── useSupabaseStorage.ts     # File uploads
│   ├── useSupabaseRealtime.ts    # Real-time subscriptions
│   ├── useSoroban.ts             # Contract interactions
│   └── useStellarAccount.ts      # Account queries
│
├── lib/                           # Utility Libraries
│   ├── config.ts                 # Environment config
│   ├── contracts/                # Contract clients
│   │   ├── bindings.ts          # Binding loader utilities
│   │   ├── donation.ts          # Donation contract client
│   │   └── podPoap.ts           # POD POAP contract client
...
4. On sign-in → Supabase creates session
...
8. On success → donation recorded in Supabase
